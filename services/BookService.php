...
): array
{
    ...
    if (count($erreurs) > 0) {
        return array('succes' => false, 'erreurs' => $erreurs);
    }
    ...
    if ($ok) {
        return array('succes' => true, 'erreurs' => []);
    }

    return array('succes' => false, 'erreurs' => ["Erreur lors de la modification du livre."]);
}
